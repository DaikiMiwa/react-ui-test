...
screens:
  ...
  calendar-log:
    ...
    layout: "Centered 430px by 820px phone frame with shared header, a raised month calendar, selected-day summary, plan/actual segmented switch, a plan panel for selected-day or next-up tasks, and actual editors modeled after the dedicated Meal Input and Workout Set Log screens."
    ...
    interactionRules: "Tapping a day changes the selected date and loads that day's mock log and plan. Future days must only show plan mode because actual records cannot exist yet. Today and past days can switch between plan and actual modes. The lower content must change between plan and actual modes: plan mode shows upcoming training and meal tasks plus adjustment actions; actual mode shows retrospective meal/training editors. The meal editor must preserve meal picker, P/F/C inputs, memo, Draft/Saved state, and Save Meal action. The training editor must preserve exercise selection, set rows, planned vs actual values, done checks, + Set, and Complete actions."
...
- 選択日サマリーは `calendar-day-summary` として、保存済みmeal数と完了セット数を `StatusPill` で表示する。ここは詳細説明ではなく、記録有無をすぐ判断するための短い状態面にする。
- 選択日サマリーの直後に `予定 / 実績` の切替を置く。未来日は実績を登録できないため `予定` のみ表示する。今日と過去日は実績へ切り替えられるようにする。
- 予定モードでは `calendar-plan-card` を置く。選択日に予定があればその日の予定を、なければ次に来る予定を `NEXT UP` として表示する。
- 実績モードでは `食事 / トレーニング` の二段目切替を置き、Meal Input / Workout Set Log に近い後追い入力UIを表示する。予定カードと実績入力を同じ下部領域に同時表示しない。
...
- トレーニング側の `calendar-retro-editor` は Workout Set Log と同じ構造に寄せる。Exercise picker、種目名、target、`SET / PLAN / ACTUAL / DONE` の列、重量と回数の実績入力、完了チェック、`+ Set`、`Complete` を含める。
- `calendar-plan-card` は自由文ではなく構造データを表示する。トレーニング予定は `exercise.name`, `target`, `sets`、食事予定は `meal.label`, `calories`, `protein`, `fat`, `carbs` を基本にする。生成が難しい長文の優先事項、自然文メニュー、自由フォーマットの調整メモは予定カードの正本にしない。
